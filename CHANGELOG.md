...
- **OpenTelemetry GenAI instrumentation** — opt-in traces/metrics over OTLP following the GenAI semantic conventions. The lightweight `opentelemetry-api` ships in the base install (zero-overhead no-op when disabled); the SDK + OTLP exporter are an opt-in extra (`pip install cli-agent-orchestrator[otel]`). See [docs/otel-deployment.md](docs/otel-deployment.md).
