...
# route discovered by inspecting ``api/main.py`` on this fork. Where a
...
def _route_command(kind: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Route map (verified against ``api/main.py`` on this fork):
    ...
    * ``pause`` / ``resume`` -> no corresponding terminal routes exist on this fork,
    ...
    * ``shutdown_session`` -> ``DELETE /sessions/{name}``.
    ...
    try:
        ...
        if kind in ("pause", "resume"):
            # No pause/resume terminal routes exist on this fork. Return a
            # structured unsupported result rather than inventing endpoints.
            return {
                "success": False,
                "kind": kind,
                "error": "unsupported",
                "detail": f"{kind} has no corresponding Backplane route on this fork",
            }
