...
def _agui_enabled() -> bool:
    ...
    Enabled by **either** of two flags, by design:

    - ``CAO_AGUI_ENABLED`` — the dedicated flag, so the AG-UI stream can be
      turned on independently of the MCP Apps iframe surface; and
    - ``CAO_MCP_APPS_ENABLED`` — because the two surfaces are fed by the *same*
      in-process event source (``SseBus``/``EventLogPublisher``), so enabling
      the iframe surface implicitly makes the AG-UI projection of that same
      stream available rather than standing up a second, parallel pipeline.

    Both flags default off, so with **neither** set this returns ``False`` and
    every AG-UI route 404s — preserving the "byte-identical with no new flags"
    contract (there is no third, always-on path).
    """

    if os.environ.get("CAO_AGUI_ENABLED", "").strip().lower() in ("1", "true", "yes"):
        return True
    return _mcp_apps_enabled()
...
async def agui_stream(
    ...
    access_token: Optional[str] = Query(
        ...
        description=(
            ...
            "(the same pattern as the WebSocket cao.bearer subprotocol)."
        ),
    ),
):
    ...
    if is_auth_enabled():
        ...
        # extract_scopes_from_token raises on malformed/expired/invalid JWTs
        # (jwt.PyJWTError and friends). Catch broadly and fail closed with a
        # clean 401 rather than letting it surface as an opaque 500 — no data
        # is served either way, but the auth telemetry stays accurate.
        try:
    ...
    async def event_generator():
        ...
        # frames so a reconnecting client resumes without gaps (it dedupes by
        # event id). The ring buffer is metadata-only, same as the live path.
        if since:
            try:
...
def _extract_ws_scopes(websocket: WebSocket) -> Optional[List[str]]:
    ...
    for proto in websocket.scope.get("subprotocols") or []:
        ...
        try:
            ...
            # HTTPException (missing config) or any jwt.PyJWTError raised on a
            # malformed/expired/invalid token: fail closed. The caller closes
            # the handshake 4401 rather than surfacing an opaque error.
            return None
    return None
...
def main():
    ...
    # Scrub bearer tokens carried in the query string (e.g. the AG-UI stream's
    # ?access_token=<jwt>) from uvicorn's access log so a captured log line
    # can't be replayed until token expiry. Access logging otherwise stays on.
