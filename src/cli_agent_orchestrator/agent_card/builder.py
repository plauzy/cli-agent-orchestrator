...
Produces an A2A v1.0-conformant ``agent-card.json`` document from CAO's
runtime state — provider list, MCP tool surface, and operator-supplied
metadata stored in ``settings.json``.
...
def build_agent_card(
    ...
    *,
    ...
    skills = [
        ...
        {"id": "code", "description": "Code generation via a serialized write queue."},
        {"id": "review", "description": "Independent N=3 majority-vote review."},
        {"id": "search", "description": "Breadth-first read across parallel workers."},
    ]
