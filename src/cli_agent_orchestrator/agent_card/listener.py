...
Phase 1 / commit 6: the Agent Card needs to be discoverable by external
A2A peers, but the main :9889 API is locked behind ``TrustedHostMiddleware``
to preserve CAO's localhost-only security stance for arbitrary requests.
The compromise (per the v2.5 plan) is a second uvicorn server bound to
:9890 that hosts only the agent_card router — no other routes, no Host
restriction, read-only.

Operators who want to keep the listener loopback-only set
``CAO_AGENT_CARD_HOST=127.0.0.1``. The default of ``0.0.0.0`` matches
the design intent (external discoverability).
"""
...
async def start_agent_card_listener(
    ...
    *,
    ...
    """
    bind_host = host or os.environ.get("CAO_AGENT_CARD_HOST", "0.0.0.0")
