...
def affected_sessions(
    ...
    *,
    ...
    ``SKILL.md`` from disk mid-session**: Kiro CLI resolves the
    ``skill://{SKILLS_DIR}/**/SKILL.md`` glob written by ``install_service``, and
    OpenCode reads through the ``OPENCODE_CONFIG_DIR/skills`` symlink. Neither
