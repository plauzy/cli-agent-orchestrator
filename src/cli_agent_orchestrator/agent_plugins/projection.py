...
one-function change and **cannot** cover the third. Kiro receives exactly one
``skill://{SKILLS_DIR}/**/SKILL.md`` glob and OpenCode's ``skills`` entry is a
...
def rebuild_projection(
    ...
    *,
    ...
    swept = _sweep(store, target_dir, prior, materialized)
...
def _sweep(
    ...
    * an entry the previous projection owned that the current one does not
      (the plugin was removed, or lost a collision), and
...
def _write_back(
    ...
    for record in records:
        ...
        if owned == tuple(record.projected_skill_names):
            continue
...
def sweep_dangling_projections(
    ...
    *,
    ...
    target_dir = _skills_dir(skills_dir)
    try:
        return _sweep(store, target_dir, previous={}, current={})
