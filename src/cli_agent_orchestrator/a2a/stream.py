"""A2A v1.0 streaming + REST polling endpoints (Phase 5 / commit 27).

Layered on top of the JSON-RPC endpoint from commit 26. Two new routes:
...
ships an in-process pub-sub primitive: callers (the dispatch wiring
in a follow-up commit) post state transitions, the SSE handler
forwards them. A ``NullBus`` is provided for tests + deployments
where streaming isn't wired.

The bus + store + transport endpoints together form the full A2A
v1.0 server side. Authentication via the JWKS published at
``/.well-known/jwks.json`` is layered on next; commit 27 ships
