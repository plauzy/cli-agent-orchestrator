...
Phase 5 / commit 26. The A2A spec mandates a task lifecycle: peers
``send`` a task, ``get`` its current state, and ``cancel`` it before
it terminates. The lifecycle requires a store that survives at least
the lifetime of one HTTP request → next.
...
short timeframes, and the Phase 1 WAL already provides durable audit.
