"""A2A v1.0 transport endpoints (Phase 5 / commit 26+).

Layered on top of the Phase 1 Agent Card listener (`:9890`). Implements
the JSON-RPC 2.0 RPC method surface (`task.send`, `task.get`,
`task.cancel`); SSE streaming + REST polling fallback land in the
following commit.
"""
