"""A2A v1.0 JSON-RPC 2.0 endpoint (Phase 5 / commit 26).

Mounted on the existing :9890 Agent Card listener so external A2A peers
discover and call CAO over the same boundary. The Phase 1 design
intentionally kept :9890 read-only (Agent Card + JWKS); commit 26 turns
it read-write by adding ``POST /a2a/v1/rpc`` for JSON-RPC 2.0 task
operations.
...
Methods:
  ...
  * ``task.send`` — peer submits a task. CAO assigns it the
    ...
    work transitions through ``working`` → terminal state. CAO's
    actual dispatch (Phase 3 router → Refinery → Polecat) is
    deferred to a follow-up commit; commit 26 ships the protocol
    surface only.
...
Authentication is layered on top in the next commit (commit 27)
once the SSE stream + REST polling endpoints are in place — the
JWKS published at ``/.well-known/jwks.json`` is the verifier.
...
def build_a2a_router(
    ...
    * ``bus`` — event bus that the SSE stream endpoint (commit 27)
      consumes from. RPC handlers publish state transitions on this
