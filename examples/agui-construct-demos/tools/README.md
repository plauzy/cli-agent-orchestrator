...
(`examples/agui-*/run.sh`) in offline / synthetic mode — no live provider,
...
|---|---|---|
| `agui-supervisor-dashboard` | `SupervisorDashboardStream` | `examples/agui-supervisor-dashboard/run.sh` |
| `agui-session-timeline` | `MultiAgentSessionTimeline` | `examples/agui-session-timeline/run.sh` |
| `agui-handoff-approval` | `AgentHandoffWithApproval` | `examples/agui-handoff-approval/run.sh` |
| `agui-cross-provider-sync` | `CrossProviderStateSync` | `examples/agui-cross-provider-sync/run.sh` |
