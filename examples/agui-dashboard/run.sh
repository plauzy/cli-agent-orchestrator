...
# leaves it running so you can either open the dashboard PWA (cao_pwa/) or run
# ./showcase.sh against the *live* server. Ctrl-C tears everything down.
#
...
Or open the dashboard PWA (separate terminal):
  cd cao_pwa && npm install && npm run dev
  # then add the instance URL ${BASE} in the picker
