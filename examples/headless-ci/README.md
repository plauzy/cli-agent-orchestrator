...
This example shows how to drive a single CAO agent from a CI runner — non-interactively, with a hard timeout, and with an exit code CI can act on. It uses the same `--headless --async --yolo` flag set documented in the [Headless Launch](../../README.md#headless-launch) section of the root README, wrapped in a polling loop.
...
- [`ci_developer.md`](ci_developer.md) — minimal `role: developer` profile tuned for one-shot non-interactive execution. Always writes a result file, never asks follow-up questions.
...
- [README → Headless Launch](../../README.md#headless-launch) — flag reference.
- [README → Session Management](../../README.md#session-management) — the underlying `cao session ...` commands the script polls.
