...
- ``test/e2e/test_websocket_auth.py`` — ``_create_terminal_authed`` +
  ``_cleanup_terminal_authed`` (auth path)
...
unavailability skip (not a fixture failure). This matches the behaviour in
W1's ``cao_terminal`` and W2's ``_create_terminal_authed``, ensuring e2e
tests that require real CLIs degrade gracefully in CI environments where
those CLIs aren't installed.
"""
...
class TerminalHandle:
    ...
    window_name : str
        tmux window name; used for pane-width introspection in W2/W3 tests.
    server_url : str
...
class TerminalFactory:
    ...
    message) rather than fixture errors, matching the existing W1/W2 contract.

    All created terminals get a unique ``w5-<hex>`` session name prefix so
