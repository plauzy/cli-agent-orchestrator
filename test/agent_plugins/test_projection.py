...
def owning_plugin(skill_name: str, store) -> str | None:
    """Which plugin's record claims ``skill_name`` — a LOCAL test oracle.

    Deliberately local rather than imported from production. This is the oracle
    the projection tests check the projection against, and an oracle that shares
    an implementation with the thing under test proves nothing: a bug in the
    shared reader would make both sides agree and the test pass. It replaced
    ``agent_plugins/provenance.py``, which existed only to be imported here --
    review 5074181308 on #584 identified it as dead production code whose
    docstring named three consumers that never called it.
    """
    for record in store.list_installed():  # already sorted by plugin name
        if skill_name in record.projected_skill_names:
            return record.name
    return None
...
class TestPluginVersusPluginCollision:
    ...
    def test_lexicographically_smallest_plugin_wins(self, store, skills_dir, tmp_path):
        ...
        assert owning_plugin("shared", store) == "alpha"
    ...
    def test_installing_an_earlier_claimant_reassigns_and_warns(self, store, skills_dir, tmp_path):
        ...
        assert owning_plugin("shared", store) == "zeta"
    ...
    def test_install_timestamp_is_not_the_tiebreaker(self, store, skills_dir, tmp_path):
        ...
        assert owning_plugin("shared", store) == "aaa"
...
class TestSweepAndDanglingLinks:
    ...
    def test_dangling_link_is_swept_without_a_rebuild(self, store, skills_dir, make_plugin):
        ...
        sweep = sweep_dangling_projections(store, skills_dir=skills_dir)

        # Return type widened to `DanglingSweep` by review 4 item 1 on #584: a bare
        # list could not distinguish "swept nothing" from "declined, store busy".
        assert "alpha" in sweep.swept
...
def test_property_winner_is_invariant_under_install_order(tmp_path_factory, names):
    ...
    for index, order in enumerate(permutations(names)):
        ...
        assert owning_plugin("shared", store) == expected, order
...
def test_property_preexisting_always_beats_every_plugin(tmp_path_factory, preexisting, names):
    ...
    if preexisting:
        assert owning_plugin("shared", store) is None
        assert "pre-existing" in (sdir / "shared" / "SKILL.md").read_text(encoding="utf-8")
    else:
        assert owning_plugin("shared", store) == min(names)
