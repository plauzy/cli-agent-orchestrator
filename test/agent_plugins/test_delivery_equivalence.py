...
against each provider's *real* delivery artifact, not against ``list_skills()``
standing in for all of them. Two of the seven (Kiro, OpenCode) never call
``list_skills()`` at all, so a test that only checked it would prove nothing
about them.
"""
...
def reachable_skill_names(provider: str, skills_dir: Path, opencode_dir: Path) -> Set[str]:
    ...
    * OpenCode → traversal of its ``skills`` config symlink.
    ...
    if provider == ProviderType.KIRO_CLI.value:
        # The literal resource Kiro receives: skill://{SKILLS_DIR}/**/SKILL.md
...
def test_every_provider_reaches_the_full_union(delivery_world, provider):
    ...
    if provider not in (ProviderType.KIRO_CLI.value, ProviderType.OPENCODE_CLI.value):
        # Kiro's glob and OpenCode's symlink are both rooted at SKILLS_DIR, so
        # neither sees `extra_dirs` — that is pre-existing CAO behaviour for
        # every skill, plugin-provided or not, and not something projection
        # changes. The catalog-based providers do see them.
...
def test_property_delivery_equivalence_across_providers(
    ...
    above rather than here: Kiro's glob and OpenCode's symlink are both rooted
    at ``SKILLS_DIR`` and so never see extra dirs — pre-existing CAO behaviour
    for every skill, not something projection changes. This property is about
    the term projection *does* control.
    """
    from cli_agent_orchestrator.utils.opencode_config import ensure_skills_symlink
