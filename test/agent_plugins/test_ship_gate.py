...
class TestCliGate:
    ...
    def test_help_never_advertises_the_group(self, monkeypatch, state):
        """The gate does not un-hide the verb in either state.

        M1 is now DECIDED (Option A, `cao plugin`), which closes Requirement 16.5's
        precondition but is not itself a decision to ship: the default stays off and
        flipping it is a separate change. So this assertion is unchanged by that
        decision -- it pins gate behaviour, not the naming question.
        """
        if state == "on":
            monkeypatch.setenv(ENV_VAR, "1")
        else:
