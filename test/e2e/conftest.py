...
``test.fixtures.cao_server`` (see W1 from PR #23 follow-up roadmap). Tests
added after W1 should consume ``cao_server`` / ``cao_terminal`` directly.
The ``API_BASE_URL`` patch loop below is a back-compat shim for the 12
pre-W1 modules that import the constant at module top.
...
def require_cao_server(cao_server: CaoServer):
    ...
    time by the 12 pre-W1 e2e modules, so their helper f-strings resolve
