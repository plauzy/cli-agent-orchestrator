"""Async WebSocket helper for the W2 integration smoke tests.

Wraps :func:`websockets.asyncio.client.connect` to:

- Add the ``cao.bearer.<token>`` subprotocol when a token is supplied
  (see ``docs/rfc/cao-auth0-websocket-2026-05-11-v1.md`` for the wire
  contract — browsers can't add custom headers to the WS handshake, so
  the JWT travels in the ``Sec-WebSocket-Protocol`` slot).
