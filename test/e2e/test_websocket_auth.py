"""WebSocket terminal auth integration smoke (4 scenarios).

Run with::

    uv run pytest -m e2e test/e2e/test_websocket_auth.py -v

Reuses the managed-server fixtures (``cao_server``, ``cao_server_with_auth``) and the
``cao_terminal_mock`` (mock_cli, Tier 2 — no external credentials). All
four scenarios run without real provider CLIs so the suite is CI-safe.

References
----------
- ``src/cli_agent_orchestrator/api/main.py`` — ``terminal_ws``.
"""
...
# Wall-clock budgets. The acceptance criterion is "all 4 scenarios pass
...
def _create_terminal_authed(
    ...
    *,
    ...
    Mirrors the body of the ``cao_terminal`` fixture but adds the
    ``Authorization`` header. Skips cleanly when the provider can't boot,
    matching that contract.
    """
    requested = f"caotest-{uuid.uuid4().hex[:12]}"
    # No client-side read timeout — provider initialization (kiro_cli, etc.)
    # can take 60s+ on cold caches; the ``cao_terminal`` fixture follows the same
...
async def test_default_off_connects_without_subprotocol_and_echoes(
    ...
    async with ws_connect(url) as ws:
        ...
        marker = f"wsdefault{uuid.uuid4().hex[:8]}"
...
async def test_auth_enabled_no_token_closes_4401(
    ...
    terminal lookup (api/main.py:1287).
    """
...
async def test_auth_enabled_malformed_token_closes_4401(
    ...
    Exercises the token-parse failure path end-to-end: the
...
def authed_terminal(
    ...
    module — does *not* extend the fixture surface.
    """
...
async def test_read_only_viewer_drops_input_and_warns_once(
    ...
    async with ws_connect(url, token=token) as ws:
        ...
        marker = f"wsviewer{uuid.uuid4().hex[:8]}"
        await ws.send(json.dumps({"type": "input", "data": f"echo {marker}\n"}))
...
async def test_full_operator_echoes_input_and_resize_triggers_sigwinch(
    ...
    async with ws_connect(url, token=token) as ws:
        ...
        # Echo check.
        marker = f"wsop{uuid.uuid4().hex[:8]}"
        await ws.send(json.dumps({"type": "input", "data": f"echo {marker}\n"}))
        seen = await _read_until(ws, contains=marker.encode(), deadline=_ECHO_DEADLINE)
        assert (
