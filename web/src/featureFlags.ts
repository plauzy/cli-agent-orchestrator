 ...
 * client, and the `/plugins/*` endpoints are all built and tested; flipping this
 * constant to `true` is the whole change once M1 lands.
 *
 ...
 * CLI group remain reachable for maintainers, mirroring the CLI group's
 * `hidden=True` and the TUI rows' `Policy::Hidden`: not advertised, still usable.
 */
