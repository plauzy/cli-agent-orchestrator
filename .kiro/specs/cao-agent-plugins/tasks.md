...
> This task list was derived from `design.md` (commit `8240ac6`, branch `spec/cao-agent-plugins`) and `requirements.md` (derived from that same `design.md` revision) as they exist in this repository as of this writing, on branch `spec/cao-agent-plugins` at commit `41781be`.
> **Issue of record:** [awslabs/cli-agent-orchestrator#573](https://github.com/awslabs/cli-agent-orchestrator/issues/573), audited against issue state `open`, `updated_at` `2026-08-07T22:43:28Z`, comment count `1`, latest comment id `5222928849`.
> **Re-check requirement:** before this task list is next revised, re-read `design.md`, `requirements.md`, and issue #573 **and all of its comments** in full. Maintainer input on **M1–M4** may have changed decisions this plan deliberately leaves open (see "Naming/decision gates" below); re-diff against the anchor rather than assuming this plan is current.
> This plan does not create or modify `design.md` or `requirements.md`, and does not resolve M1–M4.
