...
Fable's stated strengths are multi-stage planning, sub-agent delegation, and checking its
own work. That is the shape of a spec audit.

**One warning specific to this model.** Fable 5 automatically routes queries flagged by
its cybersecurity safeguards to Opus. This spec contains an XSS control (FR-6.6) and a
property that generates hostile injection strings (Property 4: `javascript:` URIs,
unterminated tags, attribute-escape attempts). That is defensive work — asserting a public
docs page escapes untrusted input — but an automated router may not draw the distinction.
If reasoning depth shifts noticeably around Property 4, you have probably been rerouted.
Frame it as output-escaping verification rather than attack construction, and say in your
report if it happened, because it would make that section non-comparable against the
Gemini run.

**Companion doc:** `HANDOFF-gemini-pro.md` carries the same task for Gemini Pro. The
bodies are deliberately near-identical so the two outputs can be diffed as an A/B. Do not
