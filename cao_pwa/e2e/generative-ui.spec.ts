...
const REPLAY = pathToFileURL(resolve(__dirname, "../demo/generative-ui-replay.html")).href;
...
test("generative UI renders uniformly across heterogeneous providers and refuses off-list components", async ({ page }) => {
  ...
  await expect(page.getByText("Which architecture for the cache layer?")).toBeVisible({ timeout: 15_000 }); // q_cli
  ...
  await expect(page.locator(".prov", { hasText: "q_cli" }).first()).toBeVisible();
