...
> **Open maintainer decisions:** **M1**–**M4** (see [Naming and Namespacing](#naming-and-namespacing)) are deliberately left open for maintainers; the ship gates that depend on them are called out where they apply.
...
> **Deviation from this design, implemented deliberately: each package additionally carries client compatibility overlays.** The layout above lists only the standard's files; the built packages also contain `.claude-plugin/plugin.json` (identity fields mirrored from `plugin.json`) plus, for the operator package, `.mcp.json` and `mcp_config.json`, each byte-identical to `mcp.json`. Both gaps were measured, not assumed. **Claude Code 2.1.226** discovers `skills/` from the standard layout unchanged, but reads package identity only from `.claude-plugin/plugin.json` and MCP servers only from `.mcp.json` — its strict `claude plugin validate` rejects the root-manifest layout outright while the marketplace install path tolerates it; with the overlay, the package installs and mounts natively (skills registered, `cao-ops` tools live in-session). **Antigravity CLI 1.1.11** (`agy plugin install`) reads identity and skills from the untouched portable layout — closer to native Agent Plugins support than any other measured client — but reads plugin MCP servers only from a root `mcp_config.json` (its own binary documents "MCP Servers defined in `plugins/<name>/mcp_config.json`"); neither the portable `mcp.json` nor Claude Code's `.mcp.json` is consulted. Agent-Plugins-conformant clients are unaffected by all of this: §6.1's fixed component locations do not include these entries, and the pinned validator emits no finding for them. The spec-pure alternative — per-client §8.2 extension directories — was rejected because neither client reads extension directories, which would make the overlay decorative. All overlay files are rendered by `build_agent_plugin.py` from the same `PackageConfig`/`render_mcp` sources as the standard files and sit under the same `--check` drift guard, so no two manifests can disagree. Full evidence transcripts: [`client-validation.md`](client-validation.md).
