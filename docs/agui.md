...
A client that drops the connection can reconnect with
`?since=<last event timestamp>` and resume without a gap: the server registers
the live subscription before replaying buffered history, then drains both with
event-id deduplication, so events published during the reconnect handoff are
neither lost nor double-delivered. Pair this with a capped exponential backoff
on the client.
...
**Events stop after a proxy idle timeout** — reconnect with backoff and resume
