...
- **Authentication**: Run `gemini` and follow the OAuth flow, or set `GEMINI_API_KEY`. For Google Cloud MCP servers (e.g., `developer-knowledge`), configure **Application Default Credentials (ADC)** via `gcloud auth application-default login`.
- **tmux 3.3+**
...
There are three ways to save or export your current Gemini CLI session:

1.  **Manual Transcript Export (`/chat share`)**:
    The built-in command exports the full conversation history to a file. It only supports `.md` and `.json` extensions.
    ```bash
    /chat share transcript.md
    /chat share session.json
    ```

2.  **Automated Script (`gemini-save-session.sh`)**:
    A custom script is provided in `~/bin/` that automatically finds the latest session file in Gemini's temporary storage and saves it to a timestamped log directory.
    To use it, run it as a shell command within the CLI:
    ```bash
    !~/bin/gemini-save-session.sh
    ```
    This script saves the transcript to `/Volumes/workplace/.logs/gemini/` and displays a macOS desktop notification.

3.  **Manual File Access**:
    Gemini CLI saves every session as a JSON file in a project-specific temporary directory.
    - **Path**: `~/.gemini/tmp/<project_hash>/chats/`
    - In this workspace, your project hash is `workplace`, so the files are in `~/.gemini/tmp/workplace/chats/`.
